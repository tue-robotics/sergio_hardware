/***************************************************************************
 tag: Ton Peters  Oktober 2014  DynamixelControllers.hpp
...
#include "DynamixelControllers.hpp"
...
DynamixelControllers::DynamixelControllers(const std::string& name) :
	...
	log(Debug) << "DynamixelControllers constructor done." << endlog();
}

bool DynamixelControllers::configureHook() {
	// Get the number of dynamixels
	n_dynamixels = dynamixel_ids.size();

	// Check if the properties are set correctly
	if ( n_dynamixels < 1 ){
		log(Error)<<"DynamixelControllers: propertie dynamixel_ids invallid!" << endlog();
		return false;
	}
	...
	for ( uint i=0; i<n_dynamixels; i++ ){
		if ( dynamixel_ids[i] < 0 || dynamixel_ids[i] > 253 ){
			log(Error)<< "DynamixelControllers: Id "<<dynamixel_ids[i]<<", property dynamixel_id invallid (range 0~253)!" << endlog();
			return false;
		}
		...
		if ( dynamixel_max[i] < dynamixel_min[i] )
			log(Warning)<< "DynamixelControllers: Id "<<dynamixel_ids[i]<<", property dynamixel_max < dynamixel_min ???" << endlog();
	}
	...
	log(Debug) << "DynamixelControllers configuration done." << endlog();
	return true;
}

bool DynamixelControllers::startHook() {
	for ( uint i=0; i<n_dynamixels; i++ ){
	...
	log(Debug) << "DynamixelControllers start done." << endlog();
	return true;
}

bool DynamixelControllers::readReference() {
	sensor_msgs::JointState goalPos;
	//goalPos = sensor_msgs::JointState();
	if (goalPosPort.read(goalPos) == NewData) {
		log(Debug) << "DynamixelControllers: new dynamixel reference obtained."<< endlog();
		// ToDo: don't hardcode
		// Check just to be sure
		for ( uint i=0; i<n_dynamixels; i++ ){
			if (goalPos.name[i] != currentPos.name[i] )
			{
				log(Error)<<"Head controller: JointState message should contain "<<currentPos.name[i]<<", while this is now "<<goalPos.name[i] <<endlog();
			}
...
void DynamixelControllers::updateHook() {
	...
	if (commStatus == COMM_RXSUCCESS){
		...
		switch (state) {
			case 1:
				log(Debug) << "DynamixelControllers: setting speeds to default and position to offset" << endlog();
				for ( uint i=0; i<n_dynamixels; i++ ){
					dynamixel_speed[i] = dynamixel_speed_default[i];
				...
				break;
			case 2:
				log(Debug) << "DynamixelControllers: Requesting current dynamixel position"<< endlog();
			...
			case 3:
				...
				log(Debug) << "DynamixelControllers: dynamixel id, " << read_dynamixel+1 << ", received new position, " << newPosition << endlog();
				currentPos.position[read_dynamixel] = (newPosition-dynamixel_offset[read_dynamixel])/RAD_TO_STEP;
				currentPosPort.write(currentPos);
				read_dynamixel ++;
				if ( read_dynamixel < n_dynamixels ){
					log(Debug) << "DynamixelControllers: Read position of dynamixel, id " << read_dynamixel+1 << endlog();
					state = 2;
				}
				else {
					read_dynamixel = 0;
					if (readReference()){
						log(Debug) << "DynamixelControllers: New reference recieved, sending goals" << endlog();
						state++;
					} else {
						state = 2;
						log(Debug) << "DynamixelControllers: Read position of dynamixel, id " << read_dynamixel+1 << endlog();
				...
				break;
			case 4:
				log(Debug) << "DynamixelControllers: setting speed and goal position" << endlog();
				create_pos_goal_packet();
				dxl_tx_rx_packet();
				state++;
				break;
		...
		log(Debug) << "DynamixelControllers: status corrupt. Resending the instruction."<< endlog();
		state = pstate;
		commStatus = COMM_RXSUCCESS;
	} else if (commStatus == COMM_RXWAITING) {
		if (trial>TRIAL_MAX) {
			log(Debug) << "DynamixelControllers: waited too long for status. Resending the instruction."<< endlog();
			state = pstate;
			commStatus = COMM_RXSUCCESS;
		} else {
			dxl_rx_packet();
...
void DynamixelControllers::create_pos_goal_packet(void){
...
void DynamixelControllers::dxl_tx_rx_packet(void) {
	dxl_tx_packet();
...
void DynamixelControllers::dxl_tx_packet(void)
{
...
void DynamixelControllers::dxl_rx_packet(void) {
	if (gbInstructionPacket[ID] == BROADCAST_ID) {
		log(Debug) << "DynamixelControllers: broadcast id, no status packet"<< endlog();
		commStatus = COMM_RXSUCCESS;
		return;
	}

	if (!(gbInstructionPacket[INSTRUCTION] == INST_READ)) {
		log(Debug) << "DynamixelControllers: not read instruction, no status packet"<< endlog();
		commStatus = COMM_RXSUCCESS;
		return;
	}
	
	if (!(statusPort.read(status) == NewData)) {		
		log(Debug) << "DynamixelControllers: waiting for data"<< endlog();
		...
		if ((gbStatusSize >= 6) && (gbStatusPacket[0] == 0xff) && (gbStatusPacket[1] == 0xff) && dxl_check_rxpacket_checksum()) {
			if ((gbStatusPacket[ID] != gbInstructionPacket[ID]) || (gbStatusSize != tmpLength)){
				log(Debug) << "DynamixelControllers: not my status packet or length not valid. waiting another"<< endlog();
				commStatus = COMM_RXWAITING;
			} else {
				log(Debug) << "DynamixelControllers: status packet ok"<< endlog();
				printErrorCode();
				commStatus = COMM_RXSUCCESS;
			}
		} else {
...
void DynamixelControllers::dxl_set_txpacket_id( int id )
{
	gbInstructionPacket[ID] = (unsigned char)id;
}

void DynamixelControllers::dxl_set_txpacket_instruction( int instruction )
{
	gbInstructionPacket[INSTRUCTION] = (unsigned char)instruction;
}

void DynamixelControllers::dxl_set_txpacket_parameter( int index, int value )
{
	gbInstructionPacket[PARAMETER+index] = (unsigned char)value;
}

void DynamixelControllers::dxl_set_txpacket_length( int length )
{
	gbInstructionPacket[LENGTH] = (unsigned char)length;
}

int DynamixelControllers::dxl_get_rxpacket_id(void) {
	return (int)gbStatusPacket[ID];
}

int DynamixelControllers::dxl_rxpacket_isError(void)
{
...
int DynamixelControllers::dxl_get_rxpacket_error( int errbit )
{
...
int DynamixelControllers::dxl_get_rxpacket_length(void)
{
	return (int)gbStatusPacket[LENGTH];
}

bool DynamixelControllers::dxl_check_rxpacket_checksum(void)
{
	unsigned char checksum = 0;
...
int DynamixelControllers::dxl_get_rxpacket_parameter( int index )
{
	return (int)gbStatusPacket[PARAMETER+index];
}

int DynamixelControllers::dxl_makeword( int lowbyte, int highbyte )
{
	unsigned short word;
...
int DynamixelControllers::dxl_get_lowbyte( int word )
{
	unsigned short temp;
...
int DynamixelControllers::dxl_get_highbyte( int word )
{
	unsigned short temp;
...
void DynamixelControllers::dxl_ping( int id )
{
	gbInstructionPacket[ID] = (unsigned char)id;
...
void DynamixelControllers::dxl_read_byte( int id, int address )
{
	gbInstructionPacket[ID] = (unsigned char)id;
...
void DynamixelControllers::dxl_write_byte( int id, int address, int value )
{
	gbInstructionPacket[ID] = (unsigned char)id;
...
void DynamixelControllers::dxl_read_word( int id, int address )
{
	gbInstructionPacket[ID] = (unsigned char)id;
	gbInstructionPacket[INSTRUCTION] = INST_READ;
	gbInstructionPacket[PARAMETER] = (unsigned char)address;
...
void DynamixelControllers::dxl_write_word( int id, int address, int value )
{
	gbInstructionPacket[ID] = (unsigned char)id;
	gbInstructionPacket[INSTRUCTION] = INST_WRITE;
	gbInstructionPacket[PARAMETER] = (unsigned char)address;
...
void DynamixelControllers::printErrorCode(void)
{
...
ORO_CREATE_COMPONENT(SERGIOCUSTOM::DynamixelControllers)
